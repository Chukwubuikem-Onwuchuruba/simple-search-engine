...
def search(keyword):
    ...
    for metadata in article_metadata():
        for word in metadata[4]:
            if word.lower() == keyword.lower():
                result.append(metadata[:4])
    return result
...
def unique_authors(count, metadata):
    ...
    result = []
    for article in metadata:
        if not article[1].lower() in authors and len(authors) < count:
            result.append(article)
        authors.add(article[1].lower())
    return result
...
def most_recent_article(metadata):
    ...
    for article in metadata:
        if article[2] > most_recent_timestamp:
            most_recent_article = article
            most_recent_timestamp = article[2]
...
#   metadata - article metadata
#
# Returns: list of Tuples containg (title, author) for all of the given 
#   metadata.
def title_and_author(metadata):
    result = []
    for article in metadata:
        result.append((article[0], article[1]))
    return result
...
def refine_search(keyword, metadata):
    new_metadata_title_author = set(title_and_author(search(keyword)))
    result = []
    for article in metadata:
        if (article[0], article[1]) in new_metadata_title_author:
            result.append(article)
    return result
